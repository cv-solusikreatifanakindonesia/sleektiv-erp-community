...
echo	Restoring idempiere DB from "$SLEEKTIV_HOME"/data/ExpDat.dmp
...
echo Import ExpDat
echo -------------------------------------
imp "$1"@"$ADEMPIERE_DB_SERVER":"$ADEMPIERE_DB_PORT"/"$ADEMPIERE_DB_NAME" FILE="$SLEEKTIV_HOME"/data/ExpDat.dmp FROMUSER=\("$2"\) TOUSER="$2"
