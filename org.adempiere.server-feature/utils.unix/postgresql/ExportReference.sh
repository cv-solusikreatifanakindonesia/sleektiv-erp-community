...
echo Saving database reference@"$ADEMPIERE_DB_NAME" to "$SLEEKTIV_HOME"/data/Adempiere_pg.dmp
...
export PGPASSWORD
pg_dump -h "$ADEMPIERE_DB_SERVER" -p "$ADEMPIERE_DB_PORT" --no-owner -U reference "$ADEMPIERE_DB_NAME" > "$SLEEKTIV_HOME"/data/Adempiere_pg.dmp
PGPASSWORD=
export PGPASSWORD
